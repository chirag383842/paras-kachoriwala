 ...
 * 1. "Close Shop for Today" (auto-resets the next day in IST)
 * 2. Manual author overrides
...
export function calculateStoreStatus(status?: StoreStatus | null): {
  ...
  // If author explicitly closed the shop without a specific single-day date
  ...
  // If author explicitly opened the shop
  if (status && status.is_open === true && status.closed_for_date !== ist.dateString) {
    const inHours = currentMinutes >= openMinutes && currentMinutes < closeMinutes;
    return {
      isOpen: inHours || status.is_open,
      isClosedForToday: false,
      nextOpenText: currentMinutes < openMinutes ? 'Opens Today at 7:00 PM' : 'Opens Tomorrow at 7:00 PM',
      todayScheduleDisplay: todaySchedule.display,
      statusLabel: inHours || status.is_open ? 'Open Now' : 'Closed Now',
    };
  }
  ...
  return {
    ...
    nextOpenText: nextText,
    todayScheduleDisplay: todaySchedule.display,
    statusLabel: inHours ? 'Open Now' : 'Closed Now',
  };
}
